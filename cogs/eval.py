    ...
    async def _eval(self, ctx, *, code: str):
        ...
        try:
            ...
            if silent or (response is None) or isinstance(response, nextcord.Message):
                em=nextcord.Embed(title='Eval Success :D', description='```Code ran without any errors```')
                await ctx.send(embed=em)
                del args, code
                return
            em=nextcord.Embed(title='Eval Success :o', description=f'```py\n{self.resolve_variable(response)}```')
            em.set_footer(text = f'`{type(response).__name__} | {(time() - a) / 1000} ms`')
            await ctx.send(embed=em)
        except Exception as e:
            em = nextcord.Embed(title='Eval Error ._.', description=f'```{type(e).__name__}: {str(e)}```')
            await ctx.send(embed=em)
