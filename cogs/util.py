...
class util(commands.Cog):
    ...
    async def userinfo(self, ctx, *, user: nextcord.Member = None):  # b'\xfc'
        ...
        embed.set_footer(text=self.client.user.name, icon_url=self.client.user.display_avatar)
    ...
    async def serverinfo(self, ctx):
        ...
        embed2.set_footer(text=self.client.user.name, icon_url=self.client.user.display_avatar)
    ...
    async def channelstats(self, ctx, channel: nextcord.TextChannel = None):
        ...
        embed.add_field(name="Channel Permissions", value=channel.permissions_synced, inline=True)
                ...
                async with ses.get(url) as r:
                    ...
                    try:
                        ...
                        if r.status in range(200, 299):
                            ...
                            em = nextcord.Embed(title='Emoji Success', description=f'Successfully created emoji: <:{name}:{emoji.id}>')
                            await ctx.send(embed=em)
                            await ses.close()
                        else:
                            em = nextcord.Embed(title="Emoji Error", description=f'Error when making request | {r.status} response.')
                            await ctx.send(embed = em)
                            await ses.close()
                            
                    except nextcord.HTTPException:
                        em = nextcord.Embed(title="Emoji Error", description='File size is too big!')
                        await ctx.send(embed=em)
    ...
    async def emojiremove(self, ctx, emoji: nextcord.Emoji):
        guild = ctx.guild
        if ctx.author.guild_permissions.manage_emojis:
            em = nextcord.Embed(title="Emoji Success", description=f'Successfully deleted (or not :P) {emoji}')
            await ctx.send(embed = em)
            await emoji.delete()
    ...
    async def toggle(self, ctx, *, command):
        ...
        if command is None:
            embed = nextcord.Embed(title="ERROR", description="I can't find a command with that name")
            await ctx.send(embed=embed)

        elif ctx.command == command:
            embed = nextcord.Embed(title="ERROR", description="You cannot disable this command ")
            await ctx.send(embed=embed)
